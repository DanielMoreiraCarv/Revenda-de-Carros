...
public class Est_EstoqueService {
	...
	public boolean removeEstoque(Est_Estoque e1) {
		try (Connection conexao = ConexaoBd.getConexao()) {
			PreparedStatement stmt = null;
			String sql = "DELETE FROM EST_ESTOQUE WHERE est_dsc_endereco = ? AND est_num_numeroendereco = ?";
			stmt = conexao.prepareStatement(sql);
	...
	public int addEstoque(Est_Estoque estoque) {
		...
		try (Connection conexao = ConexaoBd.getConexao()) {
			if (e1 == null) {
				PreparedStatement stmt = null;
				String sql = "INSERT INTO EST_ESTOQUE(est_dsc_endereco,est_num_numeroendereco,est_cod_filial) VALUES (?,?,?)";
				stmt = conexao.prepareStatement(sql);
